...
exports.createContract = async (req, res, next) => {
  ...
  const _createBy = req.user.email;

  //Get lastest number
  const objNewNo = await Contract.findOne(
    { _createBy: req.user.email },
    { no: 1, _id: 0 }
  ).sort({ no: -1 });
  let newNo = 1;
  if (objNewNo) newNo = objNewNo['no'] + 1;
...
exports.updateContract = async (req, res, next) => {
  ...
  const debtor = req.body.contractDebtor;
  const subInvestor = req.body.contractSubInvestors;
  const _modifiedBy = req.user.email;
...
exports.getContractLists = async (req, res) => {
  try {
    ...
    const constraint = getFilterContractQuery(req.user.email, no, title, value);

    const contract = await Contract.find(constraint)
      .where('status')
      .in(aryStatus)
      .where('pact')
      .in(aryPact)
      ...
      .populate('_signBy subInvestor._userSubInvestor')
      .exec();
...
exports.getContractListsLength = async (req, res) => {
  try {
    ...
    res
      .status(422)
      .send({ error: `Cannot fetch [getContractListsLength].} ` + e });
  }
};

exports.getInvestorRatio = async (req, res) => {
  try {
    const activeContractList = await Contract.find({
      _createBy: req.user.email,
      status: 'ทำสัญญา'
    })
    ...
    const investorRatio = sortByKeyValue(
      _(allSubInvestors)
        .groupBy('name')
        .map((data, key) => {
          return {
...
const getActiveContractListWithUpcomingAction = async email => {
  const activeContractList = await Contract.find({
    _createBy: email
  })
    .where('actions.isCompleted')
    .equals(false)
    .exec();

  //เลือกเฉพาะ Actions ล่าสุดที่จะแจ้งเตือน
  const result = _(activeContractList)
    .map(activeContract => {
...
exports.getDueContractLists = async (req, res) => {
  try {
    const result = await getActiveContractListWithUpcomingAction(
      req.user.email
    );
    ...
    res
      .status(422)
      .send({ error: `Cannot fetch [getDueContractLists].} ` + e });
  }
};
...
exports.markActionAsComplete = async (req, res) => {
  ...
  try {
    ...
    await contract.save();
    //Fetch updated data
    res.send(await getActiveContractListWithUpcomingAction(req.user.email));
  } catch (e) {
    console.log(e);
